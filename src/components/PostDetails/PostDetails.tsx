...
import CommentSection from '../Comment/Comment';
import DOMPurify from 'dompurify';
import { usePostActions, useUserProfile } from '@/src/hooks/user.hook';
...
const PostDetails: React.FC<PostDetailsProps> = ({ post, refetchPost }) => {
  const { user } = useUser();
  const { handleVote, handleFavorite, handleFollow, isVoting, isFavoriting, isFollowing } = usePostActions();
  const { data: authorProfile } = useUserProfile(post.author._id);
  const [selectedImage, setSelectedImage] = useState(post.images[0]);
          ...
          <div className="w-full lg:w-1/2 mb-6 lg:mb-0">
            ...
            <div className="flex justify-between items-center my-6">
              <div className="flex space-x-4">
                ...
                >
                  <FaThumbsUp size={24} className={`${post.userVote === 'upvote' ? 'text-blue-500' : 'text-gray-400'}`} />
                ...
                <button 
                  ...
                  className="flex items-center space-x-2 cursor-pointer"
                >
                  <FaThumbsDown size={24} className={`${post.userVote === 'downvote' ? 'text-red-500' : 'text-gray-400'}`} />
            ...
            <div className="flex items-center space-x-4 mb-6">
              ...
              <div className="flex flex-col">
                ...
                <button
                  ...
                  className="flex items-center mt-2 space-x-1 text-blue-500 hover:underline"
                >
                  {authorProfile?.isFollowedByUser ? (
                    <>
                      <FaUserMinus /> <span>Unfollow</span>
                    </>
                  ) : (
